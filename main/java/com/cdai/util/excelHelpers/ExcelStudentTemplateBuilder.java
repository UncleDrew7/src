 ...
 */
public class ExcelStudentTemplateBuilder extends AbstractExcelView{
    @Override
    protected void buildExcelDocument(Map<String, Object> model,
                                      HSSFWorkbook workbook, HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        // get data model which is passed by the Spring container
        List<ExcelStudent> listStudent = (List<ExcelStudent>) model.get("stTemplate");

        // create a new Excel sheet
        HSSFSheet sheet = workbook.createSheet("Add Student Template");
        ...
        header.createCell(3).setCellValue("Degree Type");
        ...
        for (ExcelStudent student : listStudent) {
            ...
            aRow.createCell(1).setCellValue(student.getFirstName());
            aRow.createCell(2).setCellValue(student.getLastName());
            aRow.createCell(3).setCellValue(student.getDegreeType());
            aRow.createCell(4).setCellValue(student.getGender());
            aRow.createCell(5).setCellValue(student.getEmail());
            aRow.createCell(6).setCellValue(student.getCity());
            aRow.createCell(7).setCellValue(student.getCountry());
        }
    }
